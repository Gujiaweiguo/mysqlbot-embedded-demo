...
此外，基础应用和高级应用的小助手配置（`sys_assistant.configuration` JSON 列）中现在均支持以下默认数据源字段：

| 字段 | 含义 |
|---|---|
| `default_datasource_id` | 小助手的默认数据源 ID，可在 SQLBot 小助手配置页面中设置 |
| `default_datasource_name` | 默认数据源的显示名称，由后端根据 `default_datasource_id` 自动解析 |

这两个字段在后端从 SQLBot 的 `sys_assistant.configuration` JSON 列中读取，前端在设置页的绑定概览区域展示。嵌入式小助手问数页走小助手 ID 路径时，会继承基础小助手的默认数据源配置，从而在问数对话中自动使用指定数据源，无需用户手动选择。
...
| 参数 | 含义 | 示例 |
|------|------|------|
| SQLBot 服务地址 (`domain`) | SQLBot 产品的后端部署地址 | `https://sqlbot.lanlnk.cn` |
...
|---|---|---|
| SQLBot 服务地址 | SQLBot 访问域名 | 例如 `https://sqlbot.lanlnk.cn` |
...
路径优先级说明：问数页具有智能路由机制。若"基础小助手"设置页已配置应用 ID（`base_assistant_id`），问数页将优先使用基础小助手的链路（iframe + postMessage 证书握手，type 1）进入对话，以继承其默认数据源配置；仅当未配置基础应用时才回退到本页的 APP ID + JWT（type 4）路径。
...
问数页支持两种进入模式，优先级逻辑为：**若基础小助手已配置应用 ID，走小助手路径；否则回退到本页 APP ID 路径**。

#### 路径一：小助手 ID 路径（优先）

触发条件：基础小助手设置页已配置 `base_assistant_id`。

此路径下问数页的行为：

1. 使用小助手 ID（`base_assistant_id`）作为活跃 ID。
2. 通过 iframe + postMessage 证书握手进入对话，iframe URL 格式：
   ```text
   ${sqlbotDomain}/#/embeddedPage?id=${assistantId}&online=${online}&userFlag=${uid}
   ```
3. 证书类型为 `type: 1`，证书内容从 `localStorage` 的 `sqlbot-embedded-token` 构建。
4. **不需要**调用后端 `/api/token/` 生成 JWT。
5. 此路径可继承基础小助手的默认数据源配置（`default_datasource_id` / `default_datasource_name`）。

#### 路径二：APP ID 路径（回退）

触发条件：基础小助手设置页**未**配置 `base_assistant_id`，此时回退到嵌入式小助手配置页的问数页 APP ID（`base_embedded_app_id`）。

此路径下问数页的行为：

1. 使用嵌入式小助手的问数页 APP ID（`base_embedded_app_id`）作为活跃 ID。
2. 通过 JWT 证书进入对话，iframe URL 格式：
   ```text
   ${sqlbotDomain}/#/embeddedPage?id=${chatAppId}
   ```
3. 证书类型为 `type: 4`，附带 `busiFlag: 'ds'`。
4. JWT 由后端 `/api/token/` 接口生成。

### 数据源页（advanced.vue）

数据源页保持不变，仍使用 JWT 路径：

1. 前端请求后端 `/api/token/` 获取 JWT。
2. 后端根据传入的 `appId` 读取 `advanced_embedded_app_*` 凭据生成 token。
3. 前端挂载数据源页。

### 配置页提示说明

嵌入式小助手配置页中展示了路径优先级说明：
